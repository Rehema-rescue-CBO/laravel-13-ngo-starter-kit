<div class="p-6">
    <header class="mb-6">
        ...
        <flux:subheading>{{ __('Update staff member profile, image, and bio.') }}</flux:subheading>
    </header>

    <flux:card>
        <form wire:submit="updateStaff" class="space-y-6">
            {{-- Name --}}
            ...
            {{-- Current Image --}}
            @if ($staff->image_url && !$image)
                <div>
                    <flux:label>{{ __('Current Image') }}</flux:label>
                    <div class="mt-2 relative inline-block">
                        <img src="{{ asset('storage/' . $staff->image_url) }}" class="h-40 w-40 rounded-lg object-cover shadow-md border border-gray-200 dark:border-gray-700">
                    </div>
                </div>
            @endif
