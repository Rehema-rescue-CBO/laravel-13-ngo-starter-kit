<div class="p-6">
    <header class="mb-6">
        <flux:heading size="xl">{{ __('Edit Partner') }}</flux:heading>
        <flux:subheading>{{ __('Update the details for :name.', ['name' => $partner->name]) }}</flux:subheading>
    </header>
        ...
        <form wire:submit="save" class="space-y-6">
            ...
            <div>
                ...
                <div class="mt-2 flex items-start gap-4">
                    ...
                    @elseif ($partner->image_url)
                        <img src="{{ $partner->image_url }}" class="h-32 w-auto rounded-lg object-cover shadow-sm border border-gray-200 dark:border-gray-700">
                    @endif

                    <div class="flex-1">
                        <input type="file" wire:model="image" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 dark:file:bg-gray-700 dark:file:text-gray-200" />
                        <p class="mt-1 text-xs text-gray-500">{{ __('Leave empty to keep the current logo.') }}</p>
            ...
            <div class="flex gap-2">
                ...
                <flux:button type="submit" variant="primary">{{ __('Update Partner') }}</flux:button>
            </div>
        </form>
    </flux:card>
</div>
